...
# Создаем героев
hero = Person(name='Шрек', weap='меч', hp=100, dmg=30)
...
time.sleep(2)
print('\n По пути вам встречатся сильный враг зовут его Осел раньше он был другом но теперь он предал вас...')
answer = input('Хотите ли вы сразиться?')
# Бой с врагом
if answer == 'да':
    hero.fight(enemy)
    if hero.alive:
        print('\nПосле тяжелого боя вы чувствуете прилив сил и воруете снаряжение врага')
        hero.health = 100
        hero.damage *= 3
        print('Здоровье восстановлено а сила удара теперь равняется', hero.damage)
else:
    print('\n Вы решили избежать боя но с драконом так не получится :)')

# Бой с драконом (если герой победил)
if hero.alive:
    print("\nПосле долгого пути, Шрек решает бросить вызов самому страшному чудовищу - дракону Смаугу!")
    time.sleep(2)
    hero.fight(dragon)
else:
